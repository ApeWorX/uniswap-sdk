...
from .types import BaseIndex, Route, Solution, Solver
...
class Uniswap(ManagerAccessMixin):
    ...
    def swap(
        ...
        **txn_kwargs,
        ...
        if (sender := txn_kwargs.get("sender")) and (
            allowance := Decimal(have.allowance(sender, self.universal_router.address))
            / Decimal(10 ** have.decimals())
        ) < amount_in:
            logger.warning(
                f"{sender} does not have enough allowance, have {allowance} and need {amount_in}"
            )
